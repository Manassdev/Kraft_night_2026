...
export const CarryItemScreen: React.FC<CarryItemScreenProps> = ({ navigation }) => {
  ...
  const [to, setTo] = useState('College');
  const [itemName, setItemName] = useState('Exam Hall Ticket & Notes');
  const [description, setDescription] = useState('Urgent document folder for 10:00 AM class');
  const [size, setSize] = useState<ItemSize>('Book / Document');
  const [suggestedTip, setSuggestedTip] = useState('25');
